...
跳转：单引号，或者反引号。两个单引号或者两个反引号表示跳转到最近一次跳转之前的位置。
...
如果需要同时把指定行移到shell窗口顶部（中部或者底部）则需要在上述命令后添加 zt（z. 或者 zb）  
...
8.0以后，如果没有`user vimrc`，才会把`backspace`设置为`indent,eol,start`,否则为空。
查看版本为 version 8.1.1777，将`.vimrc`文件改名后，backspace恢复正常。
